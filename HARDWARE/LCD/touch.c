...
//	tempfac=AT24CXX_ReadOneByte(SAVE_ADDR_BASE+13);//读取标记字,看是否校准过！ 		 
	if(0X0A)//触摸屏已经校准过了	tempfac==		   
	{    												 
//		tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE,4);	
//		tp_dev.xfac=(float)tempfac/100000000;//得到x校准参数
//		printf("tp_dev.xfac %f\n",tp_dev.xfac);
		tp_dev.xfac=-0.081919;tp_dev.xoff=0x014c;
		tp_dev.yfac=-0.128019;tp_dev.yoff=0x01ea;
//		tempfac=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+4,4);			          
//		tp_dev.yfac=(float)tempfac/100000000;//得到y校准参数
//		printf("tp_dev.yfac %f\n",tp_dev.yfac);
//	    //得到x偏移量
//		tp_dev.xoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+8,2);	
//			printf("tp_dev.xoff %x\n",tp_dev.xoff);
// 	    //得到y偏移量
//		tp_dev.yoff=AT24CXX_ReadLenByte(SAVE_ADDR_BASE+10,2);		
//			printf("tp_dev.yxoff %x\n",tp_dev.yoff);
  		tp_dev.touchtype=0x00;//AT24CXX_ReadOneByte(SAVE_ADDR_BASE+12);//读取触屏类型标记
		...
		{	
			...
			{			   
				...
				case 4:	 //全部四个点已经得到
					...
					//计算结果
					tp_dev.xfac=(float)(lcddev.width-40)/(pos_temp[1][0]-pos_temp[0][0]);printf("%f\n",tp_dev.xfac);//得到xfac		 
					tp_dev.xoff=(lcddev.width-tp_dev.xfac*(pos_temp[1][0]+pos_temp[0][0]))/2;printf("%x\n",tp_dev.xoff);//得到xoff
						  
					tp_dev.yfac=(float)(lcddev.height-40)/(pos_temp[2][1]-pos_temp[0][1]);printf("%f\n",tp_dev.yfac);//得到yfac
					tp_dev.yoff=(lcddev.height-tp_dev.yfac*(pos_temp[2][1]+pos_temp[0][1]))/2;printf("%x\n",tp_dev.yoff);//得到yoff  
